            ...
            if (result.HasValue && result.Value)
            {
                ...
                using (var fStream = File.OpenRead(pickedImagePath))
                {
                    try
                    {
                        ...
                        FaceServiceClient faceServiceClient = new FaceServiceClient(subscriptionKey, endpoint);
                        ...
                        foreach (var face in faces)
                        {
                            DetectedFaces.Add(new Face()
                            {
                                ...
                                Moustache = string.Format("Moustache: {0}", face.FaceAttributes.FacialHair.Moustache.ToString()),
                                Beard = string.Format("Beard: {0}", face.FaceAttributes.FacialHair.Beard.ToString()),
                            });
                        }
